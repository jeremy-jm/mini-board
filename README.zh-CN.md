...
**mini-board** 是一个单页（SPA）任务看板应用，交互上对标 Trello 的基础能力：三列状态（Todo / In Progress / Done）、卡片式任务、拖拽改状态与同列排序，数据通过后端 API 写入 **PostgreSQL**，刷新页面不丢失。
...
（更细的目录约定、API 约定与测试范围见仓库内 [`my-draft.md`](my-draft.md)。）
...
**说明**：若无法从国内网络拉取 Docker 基础镜像，可为 Docker 配置镜像加速或使用本机 Node + Postgres 开发；详见 [`my-draft.md`](my-draft.md) 中「Docker 拉取镜像失败」一节。
...
| [`my-draft.md`](my-draft.md) | 设计与实现说明（目录、API、测试策略等） |
